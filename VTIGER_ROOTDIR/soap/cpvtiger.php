...
$server->register(
	'get_cantieri_components',
	array('id'=>'xsd:string','module'=>'xsd:string','customerid'=>'xsd:string','sessionid'=>'xsd:string'),
	array('return'=>'tns:field_details_array'),
	$NAMESPACE);
	
$server->register(
	'is_ModComments_enable',
	array('id'=>'xsd:string','module'=>'xsd:string','customerid'=>'xsd:string','sessionid'=>'xsd:string'),
	array('return'=>'tns:field_details_array'),
	$NAMESPACE);
...
function get_attachments($input_array)
{
	global $adb,$log;
	$log->debug("Entering customer portal function get_attachments");
	$adb->println("INPUT ARRAY for the function get_attachments");
	$adb->println($input_array);
	...
	$id = $input_array['id'];
	$block = $input_array['block'];
	$sessionid = $input_array['sessionid'];
	$blockid = $input_array['blockid'];

	$isPermitted = check_permission($id,$block,$blockid);
	if($isPermitted == false) {
		return array("#NOT AUTHORIZED#");
	}
	...
	$query = "select vtiger_senotesrel.crmid, vtiger_attachments.*,vtiger_notes.filename,vtiger_notes.filelocationtype 
	...
	$log->debug("Exiting customer portal function get_attachments");
	return $output;
}
...
function add_attachment($input_array)
{
	...
	$log->debug("Entering customer portal function add_attachment");
	$adb->println("INPUT ARRAY for the function add_attachment");
	$adb->println($input_array);
	$id = $input_array['id'];
	$sessionid = $input_array['sessionid'];
	$blockid = $input_array['blockid'];
	...
	$setype = $block.' '.'Attachment';
	
	$crmquery = "insert into vtiger_crmentity (crmid,setype,description,createdtime) values(?,?,?,?)";
	$crmresult = $adb->pquery($crmquery, array($attachmentid, $setype, $description, $date_var));
	...
	$relatedresult = $adb->pquery($relatedquery, array($blockid, $attachmentid));
	...
	$res = $adb->pquery($tic_doc,array($blockid,$focus->id));
...
function check_permission($customerid, $module, $entityid) {
	...
	switch($module) {
		...
		case 'ProjectTask'	:	return true;
							
							break;

		case 'ProjectMilestone'	:	return true;
...
function get_cantieri_components($id,$module,$customerid,$sessionid) {
	checkFileAccessForInclusion("modules/$module/$module.php");
	require_once("modules/$module/$module.php");
	require_once('include/utils/UserInfoUtil.php');

	global $adb,$log;
	$log->debug("Entering customer portal function get_cantieri_components ..");
	$check = checkModuleActive($module);
	if($check == false) {
		return array("#MODULE INACTIVE#");
	}
	...
	if($module == 'Documents') {
		$query = "SELECT vtiger_notes . * , vtiger_crmentity.smownerid, vtiger_attachmentsfolder.foldername
			FROM vtiger_notes
			LEFT JOIN vtiger_attachmentsfolder ON vtiger_attachmentsfolder.folderid = vtiger_notes.folderid
			INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_notes.notesid
			INNER JOIN vtiger_senotesrel ON 
			(vtiger_senotesrel.notesid = vtiger_crmentity.crmid OR vtiger_senotesrel.crmid = vtiger_crmentity.crmid ) 
			WHERE vtiger_crmentity.deleted =0 AND (vtiger_senotesrel.notesid = ? OR vtiger_senotesrel.crmid = ?)";
	}	
...
function get_cantieri_list_values($id,$module,$sessionid,$only_mine='true'){
	...
	if($module == 'Cantieri'){
		$query ="select distinct vtiger_cantieri.*,vtiger_crmentity.smownerid,
		case when vtiger_cantieri.nome_cliente !=0 then vtiger_cantieri.nome_cliente else 
		vtiger_cantieri.committente end as entityid,
		case when vtiger_cantieri.nome_cliente !=0 then 'Contacts' else 'Accounts' end as setype
		from vtiger_cantieri
		left join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_cantieri.Cantieriid
		where vtiger_crmentity.deleted=0 and (committente in (". generateQuestionMarks($entity_ids_list) .") or 
		nome_cliente in  (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($module == 'Cantieri' && $fieldname == 'collaudo') {
					$cantieri = $fieldvalue;
					$cantieri_id = $adb->query_result($res, $j, 'cantieriid');
					$fieldvalue = '<a href="index.php?module=Cantieri&action=index&id='.$cantieri_id.'">'.$cantieri.'</a>';
			}
...
function get_cantieri_detail($id,$module,$customerid,$sessionid)
{
	require_once('include/utils/UserInfoUtil.php');
	require_once('include/utils/utils.php');

	global $adb,$site_URL,$log,$current_user;
	$log->debug("Entering customer portal function get_cantieri_details $id - $module - $customerid - $sessionid");
	$user = new Users();
	$userid = getPortalUserid();
	$current_user = $user->retrieveCurrentUserInfoFromFile($userid);

	$isPermitted = check_cantieri_permission($customerid,$module,$id);
	if($isPermitted == false) {
		return array("#NOT AUTHORIZED#");
	}
	...
	$fieldres = $adb->pquery($fieldquery,array(getTabid($module)));
	$nooffields = $adb->num_rows($fieldres);
	$query = "select vtiger_cantieri.*,vtiger_crmentity.*,
		vtiger_cantiericf.* from vtiger_cantieri
		inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_cantieri.cantieriid
		INNER JOIN vtiger_cantiericf ON vtiger_cantieri.cantieriid = vtiger_cantiericf.cantieriid
		where vtiger_cantieri.cantieriid=?";
	$res = $adb->pquery($query, array($id));
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		if($fieldname == 'subject' && $fieldvalue !='')
		{
			$fieldid = $adb->query_result($res,0,'invoiceid');
			//$fieldlabel = "(Download PDF)  ".$fieldlabel;
			$fieldvalue = '<a href="index.php?downloadfile=true&module=Cantieri&action=index&id='.$fieldid.'">'.$fieldvalue.'</a>';
		}
		if( $fieldname == 'salesorderid' || $fieldname == 'contactid' || $fieldname == 'accountid' || $fieldname == 'potentialid')
		{
			$crmid = $fieldvalue;
	...
	$log->debug("Entering customer portal function get_cantieri_detail ..");
	return $output;
}


function check_cantieri_permission($customerid, $module, $entityid) {
	global $adb,$log;
	$log->debug("Entering customer portal function check_permission ..");
	$show_all= show_all($module);
	$allowed_contacts_and_accounts = array();
...
function is_ModComments_enable($module) {
	
	global $adb;
	
	$query = "SELECT * FROM vtiger_fieldmodulerel WHERE relmodule =  ? AND module =  'ModComments'";
	
	$res = $adb->pquery($query,array($module));
	
	if($adb->num_rows($res)){
		$output[0]['IsActive'] = 'true';
	} else {
		$output[0]['IsActive'] = 'false';
	}
